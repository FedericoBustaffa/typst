...
#show: doc => note_template([Map], doc)

#title()

A *map* is a pattern in which a single function $F$ can be applied independently
to each element of a collection, producing an output collection of the same
cardinality.
...
The implementation can use a _farm_ skeleton, for example with a simple
schema where an emitter dispatch tasks to workers and then a collector catch the
results.
...
_lockstep_ behavior is enforced.

== Optimal Number of Workers <optimal-number-of-workers>
...
$
  T_c (n , k) & = k dot.op T_(upright("comm")) (n / k) + n / k dot.op T_F +
                T_(upright("comm")) (n / k) \
              & = (k + 1) dot.op T_(upright("comm")) (n / k) + n
                / k dot.op T_F \
...
Usually $k_(upright("opt"))$ is a very large number because $n$ is large and
$t_0$ is low.

== Binary Tree Scatter and Gather <binary-tree-scatter-and-gather>
...
#figure(
  image("images/map_tree_scatter.png", width: 45%),
  caption: [ Map Tree Scatter ],
)
...
$
  T_(upright("scatter")) = 2 dot.op sum_(i = 1)^(⌈ log_2 k ⌉)
  T_(upright("comm")) (n / 2^i)
$
...
#figure(
  image("images/map_tree_gather.png", width: 45%),
  caption: [ Map Tree ],
)
...
- In this scenario $T_c^("map") (n , p) approx n / k dot T_(F + G)$,
