...
In both cases the model have a #emph[inductive bias] that is a #emph[guess] on
which the objective function would be. In other words, by observing data or by
observing model results, we need to choose an #strong[hypothesis function] in
advance.
...
Let’s start from the #strong[regression] task, where the model, basically
receives $l$ patterns and tries to minimize the error between the hypothesis
function and the input patterns.

== Univariate Linear Regression <univariate-linear-regression>

The simplest case of linear regression is #strong[univariate];, in other words
we have a training set composed by $l$ patterns that are couples of real numbers
...
To move towards the best solution by minimizing the #strong[loss function];, and
the most popular is the #strong[mean square error] (#strong[MSE];).

$
  upright("Loss") (h_(upright(bold(w)))) = E (upright(bold(w)))
  = 1 / l sum_(p = 1)^l (y_p - h_w (x_p))^2
$
...
#note(title: "Why the mean?")[
  Minimize the #strong[total] error or the #strong[mean] gives the exact same
  line, but usually the mean error is useful for two reasons:

  - #strong[Comparison];: it’s simpler to compare with the error on a different
    ...
    $100.000$ patterns, the total error in the second case will be higher but not
    because of a bad fitting.
  - #strong[Scaling];: as anticipated before, the mean brings the total error
    back to the problem scale. This is useful for comparison but also because is
    more understandable from a human perspective. The total error is not directly
    #emph[usable] to have an idea on the model performance.
]

So we need to solve a #strong[least (mean) square] (#strong[LMS];) problem, that
is usually the standard to solve overdetermined systems. To do that we basically
need to compute the #strong[gradient] of the loss function, in order to find its
minimum
...
find the $upright(bold(w))$ such that

$
  frac(partial E (upright(bold(w))), partial w_0) & = - 2 dot.op (y - (w_1 x +
      w_0)) & = 0\ frac(partial E (upright(bold(w))), partial w_1) & = - 2 dot.op (y -
    (w_1 x + w_0)) dot.op x & = 0
$

Of course we can then perform a sum over all patterns, obtaining

$
  frac(partial E (upright(bold(w))), partial w_0) & = - 2 sum_(p = 1)^l (y_p -
                                                      (w_1 x_p + w_0)) \
                 frac(partial E (w), partial w_1) & = - 2 sum_(p = 1)^l (y_p -
                                                      (w_1 x_p + w_0)) dot.op x_p
$
...
For the #strong[multivariate] case there isn’t much more to say except that we
need a more general notation, in order to be able work with $n$-dimensional
inputs.

So from now on $x$ is a vector $upright(bold(x))$ and we want to multiply each
feature for one of the weights in order to build the hypothesis function

$
  h_(upright(bold(w))) (upright(bold(x))) = w_0 + w_1 x_1 + w_2 x_2 + dots.h.c +
  w_n x_n
$
...
$
  frac(partial E (upright(bold(w))), partial w_i) = - 2 dot.op (y - h_w (x))
  dot.op x_i = - 2 dot.op (y - sum_(i = 1)^n w_i x_i) dot.op x_i
$

because for every $j eq.not i$ the partial derivative considers every term
...
output can only be $- 1 \/ + 1$ (or any other encoding of the two labels) and so
we will obtain a line that cut the $x$ axes in a certain point.
...
So in this case our #emph[loss] function (the one we want to minimize) remains
...
And the point where our line changes sign define the so called *linear threshold
unit* (#strong[LTU];), that in general is an hyperplane orthogonal to the
#strong[decision hyperplane];.

#figure(
  image(
    "images/linear_model_binary_classification_with_boundary.png",
    width: 80%,
  ),
  caption: [ Linear model binary classification ],
)
...
fit data, and so if there is a correlation between $upright(bold(x))_p$ and the
corresponding $y_p$, data with same label should be in some way
#emph[clustered];. In this sense the model describes #emph[gradually] how the
...
- #strong[Direct];: analytical method that consists in computing the gradient,
  that, as a derivative for a univariate function, is zero where error function
  has a maxima. Setting the negative gradient to zero and solving the system gives
  us instead a minima.
- #strong[Numerical];: iterative method the uses the negative gradient in order
  to gradually move towards a minimum.
...
learning algorithm, but to solve the system the are many efficient numerical
methods that can be used.

== Numerical Method <numerical-method>

The most common way to solve the least (mean) squares problem is
#emph[iterative];, and the algorithm used is named *gradient descent
algorithm*;. This is usually preferred to the direct method because
...
The key idea is to compute the gradient of the #emph[loss] function with respect
to the weights. The gradient is basically a vector pointing in the direction of
...
This algorithm let us move towards the minimum of our #emph[loss] function by
#emph[small] steps, proportional to how far we are from the minimum and the
learning rate.

= Linear Basis Expansion <linear-basis-expansion>
...
#strong[linearly separable] problems.

In order to add some complexity we can use a technique called *linear basis
expansion* (#strong[LBE];), that now let us work with #emph[non linear]
functions. The model is still linear with respect to the weights, but now we
accept that input and output data can have some non linear relationship.
...
model risk overfitting. Other two cons are #emph[curse of dimensionality] and
the fact that $phi.alt$ is fixed, and not automatically deduce based in data.

== Tikhonov Regularization <tikhonov-regularization>

In order to control the complexity of the model, a technique called
#strong[Tikhonov regularization] can be used. The application of this method in
the case of regression lead to the so called #strong[Ridge regression];, where
the model is limited by the values of $w$.

In general high value of $w$ are penalized, in order to make some $w_j$ tend to
zero, so that less terms are used.

$
  upright("Loss") (w) = sum_(p = 1)^l (y_p - x_p^T w)^2 + lambda parallel w
  parallel^2
$
...
As we can see we extended the #emph[loss] function with a new term in which
$lambda$ is a new #emph[small] positive hyperparameter chosen in model selection
phase.

Of course we need to find a trade off between these two terms because

- with a #strong[small] $lambda$ the focus is on obtaining a small error, but
  with a too complex model the risk is overfitting ($lambda = 0$) is previous
  #emph[unregolarized] model).
- with a #strong[high] $lambda$ the second term can become dominant, increasing
