...
- Case $A^(+) A$ with $A^tack.b A$ invertible:
  $ A^(+) A = (A^tack.b A)^(- 1) A^tack.b A = I_n $
  but if $A^tack.b A$ is not invertible, it means that $A$ has not full column
  rank, and most important is not possible to solve the system.
- Case $A A^(+)$:
  $
    A A^(+) = A (A^tack.b A)^(- 1) A^tack.b = A A^(- 1) A^(- tack.b) A^tack.b =
    (A A^(- 1))^tack.b = I_m
  $
...
To check when $A^tack.b A$ is invertible we need to know if $A$ has full column
rank.

= Singular Value Decomposition <singular-value-decomposition>

Given a matrix $A = U Sigma V^tack.b$ with rank $r$ and that is _thin_, then its
