...
#show: doc => note_template([Stochastic Simulations], doc)
...
have high concentrations of reactants, the *law of large numbers* allow
...
*discrete variables* become necessary. If we consider for example the
reaction
...
The *Gillespie’s stochastic simulation algorithm (SSA)* is an exact
...
The constant $c_mu$ is used to compute the *propensity* $a_mu$ (or stochastic
rate) of $R_mu$ to occur in the whole chemical solution:
...
- Let $X_1 , dots.h , X_n$ be independent exponentially distributed random
  ...
  $ X = min (X_1 , dots.h , X_n) $

  is also exponentially distributed with parameter $lambda = lambda_1 + dots.h -
  lambda_n$. So that the algorithm can use a unique exponential distribution for
  the whole set of reactions.
